...
public function getUserByEmail($email) {
    if( $stmt = $this->conn->prepare("SELECT * FROM users WHERE
    email = ?")){
    $stmt->bind_param("s", $email);
    $stmt->execute();
    $stmt->store_result();
     ...
     */
     ...
     $stmt = $this->conn->prepare("SELECT email from users WHERE
    email = ?");
     $stmt->bind_param("s", $email);
     $stmt->execute();
     $stmt->store_result();
   ...
   // TODO
   //function to update user details (name, email, password)
